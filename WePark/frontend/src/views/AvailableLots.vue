<template>
  ...
  const currentBooking = async () => {
    try {
      ...
      if (ok) {
        console.log('Raw reservations from backend:', resData);
        // Only show bookings with no end_time (active reservations)
        currentBookings.value = resData.filter(r => !r.end_time);
      } else {
        alert(resData?.message || "Unauthorized")
  ...
  const occupySpot = async (id) => {
    try {
      ...
      if (ok) {
        ...
        alert(resData?.message || `Failed to occupy spot (Status: ${status})`)
        console.error('Occupy spot error:', resData)
      }
    } catch (err) {
      alert("Unexpected error while occupying spot.")
      console.error(err)
    }
  }
...
const releaseSpot = async (id) => {
  try {
    const { ok, resData, status } = await callApi(`payment?reservation_id=${id}`)
    if (ok) {
      order.value = resData
      // Refresh current bookings and notify ParkingHistory to refresh
      await currentBooking()
      window.dispatchEvent(new Event('parking-history-refresh'))
    } else {
      alert(resData?.message || `Failed to release spot (Status: ${status})`)
      console.error('Release spot error:', resData)
      closeModal()
    }
  } catch (err) {
    alert("Unexpected error while releasing spot.")
    console.error(err)
    closeModal()
  }
}
